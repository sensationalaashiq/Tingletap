...
- **All server-side logic runs as Netlify Functions** (`netlify/functions/`). There is no local Express API server. To invoke functions locally, run `npm run netlify:dev` (requires the Netlify CLI). In production, Netlify routes `/.netlify/functions/*` calls automatically.
...
2. **M-16 — CORS restricted to production domain**: All 22 Netlify function files now use `process.env.ALLOWED_ORIGIN || '*'` instead of a hardcoded `'*'`. ⚠️ **Action needed from you**: set `ALLOWED_ORIGIN=<your-production-domain>` in Netlify → Site settings → Environment variables to activate the restriction in production (falls back to `*` if unset, preserving dev behaviour).
...
|---|---|---|
| `R2_Public_Bucket` | *(your public bucket name)* | Public bucket name ✅ already set |
| `R2_Private_Bucket` | *(your private bucket name)* | Private bucket name ✅ already set |
| `R2_PUBLIC_BUCKET_URL` | e.g. `https://pub-xxxx.r2.dev` | **⚠️ STILL NEEDED** — permanent public URL (Cloudflare R2 dashboard → public bucket → Settings → Public Access URL) |
...
1. **Public bucket** — set "Public Access" to **Enabled** in Cloudflare R2 dashboard. Copy the "Public Bucket URL" and set it as `R2_PUBLIC_BUCKET_URL`.
2. **Private bucket** — keep Access **Private** (default).
