...
## Session 6 additions (July 3, 2026)

- **Batch React.memo wrapping safely**: before bulk-wrapping many modal/popup components in `React.memo(...)`, grep each file's default-import line (`import React from 'react'` vs named-only `import { useState } from 'react'`) — only default-imported files can use `React.memo` directly without adding an import. Also grep each file's tail to confirm the uniform `};\n\nexport default X;` closing pattern before doing search-replace across many files; always follow with a full `vite build` to catch any brace mismatch.
- **Stray dev-server process can block workflow restart**: "Port 5000 already in use" on `restart_workflow` can mean a leftover `vite` process from a previous session is still bound to the port (not necessarily the current workflow). Check `ps aux | grep vite`, `kill -9 <pid>` the stray process, then retry the restart — don't just repeat the restart call.
- **Audit-report claims can go stale**: re-verify specific audit findings against current code before "fixing" them — e.g. `subscribeAllRJEarnings` already had `limit(100)` despite an audit claiming otherwise; the fix had presumably already shipped in an earlier session. Also check whether a flagged RTDB path (e.g. `privateMessages`) is actually written anywhere in `src` before spending effort hardening its security rule — dead paths are zero-value to fix.
- **`useStableCallback` pattern for safely memoizing extracted sub-components**: instead of manually auditing every dependency of a large/complex handler (risky, easy to miss one and cause a stale-closure bug), wrap it with `const stable = useCallback((...args) => ref.current(...args), [])` where `ref.current = fn` is reassigned every render. The returned function has a permanently stable identity but always calls the latest logic — safe to pass into a `React.memo` child without dependency-array risk. Used this to extract HomePage.jsx's message-list rendering (`.filter().map()` over `messages`) into a separate `MessageList` component (still defined in the same file, next to the already-extracted `ChatMessage`) so it no longer re-computes/re-renders on every keystroke in the chat input. `ChatInput` extraction was evaluated in the same pass but deferred — its JSX is split across two non-contiguous blocks (attachment popup + footer bar) with several inline helper components, making it a separate, larger job.
