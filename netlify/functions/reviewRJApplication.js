// netlify/functions/reviewRJApplication.js
// Owner/Admin only: approve, reject, or request resubmission of an RJ application.
// On approve/reject: deletes R2 audio, updates rjApplications/{uid}, and on
// approve assigns the official 'rj' badge to users/{uid}.
// Mirrors reviewBadgeApplication.js.
...
export const handler = async (event) => {
  ...
  let app;
  try {
    app = await fsGet(`rjApplications/${applicantUid}`, token);
  } catch (e) {
  ...
  if (action === 'approve' || action === 'reject') {
    const keysToDelete = [app.introKey, app.songKey, app.welcomeKey].filter(Boolean);
    if (keysToDelete.length > 0) {
      try {
        await deleteObjects(keysToDelete);
      } catch (e) {
        console.error('[reviewRJApplication] R2 delete error:', e.message);
      }
    }
  }
  ...
  if (action === 'approve' || action === 'reject') {
    appUpdate.introKey   = null;
    appUpdate.songKey    = null;
    appUpdate.welcomeKey = null;
  }
  ...
  try {
    await fsPatch(`rjApplications/${applicantUid}`, appUpdate, token);
  } catch (e) {
    console.error('[reviewRJApplication] Firestore app update error:', e.message);
  ...
  if (action === 'approve') {
    try {
      await fsPatch(`users/${applicantUid}`, {
        badge:           'rj',
        rjVerifiedAt:    now,
        rjVerifiedBy:    reviewerName,
      }, token);
    } catch (e) {
      console.error('[reviewRJApplication] users update error:', e.message);
    }
  }
